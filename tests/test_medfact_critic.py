...
import pytest
from medfact_critic.models import FrontierPayload, ExecutionStatus
from medfact_critic.engine import FrontierDomainEngine
from medfact_critic.agents import ClinicalClaimExtractorAgent, EHRReferenceFactCheckerAgent, NLIEntailmentScorerAgent, MedFactCriticCoordinator
from medfact_critic.cli import main


def test_sub_agents():
    a1 = ClinicalClaimExtractorAgent()
    ...
    a2 = EHRReferenceFactCheckerAgent()
...
def test_coordinator():
    coord = MedFactCriticCoordinator()
    ...
    assert "Natural Language Inference (NLI) Standards" in ans or "specifications" in ans
