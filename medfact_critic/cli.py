"""
Command-Line Interface for MedFact-Critic: Clinical Fact-Checking & Citation Verification Agent.
"""
import argparse
import csv
import json
...
from .agents import MedFactCriticCoordinator

coordinator = MedFactCriticCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="clinical-llm-hallucination-critic", description="MedFact-Critic: Clinical Fact-Checking & Citation Verification Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  MEDFACT-CRITIC: CLINICAL FACT-CHECKING & CITATION VERIFICATION AGENT")
        print(f"  Domain: Clinical NLP | Standard: Natural Language Inference (NLI) Standards")
    ...
    if args.command == "batch":
        ...
        for r in rows:
            payload = FrontierPayload(
                ...
                is_critical_flag=str(r.get("is_critical_flag", "False")).strip().lower() in ("true", "1", "yes"),
            )
            dossier = coordinator.process(payload)
            row_dict = dict(r)
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting MedFact-Critic: Clinical Fact-Checking & Citation Verification Agent on http://{args.host}:{args.port}")
